...
class QrDetector():
    ...
    def readImage(self,frame):
        ...
        # print("Decoded Data : {}".format(data))
            ...
            dis,ang = self.GetDistanceAndAngle(frame,rvecs,tvecs,5.2)
            self.display(frame, rvecs,tvecs)
            return [dis,ang[0][0],ang[1][0],ang[2][0]]                                              #Return dis AngX, AngY, AngZ
            if(self.visualizeResult):
...
if (__name__=="__main__"):
    ...
    qrScan = QrDetector(visualizeResult=True,debug=False)
    while(1):
        qrScan.loop()
        if(qrScan.stop):
            qrScan.closeCap()
